...
**브랜드**는 두 단계로 찾고, 두 결과를 **구분해서** 보여 줍니다.

1. **프랜차이즈** (`brand_source = dictionary`) — `App\Support\Franchises::BRANDS` 에 등록된
   표기를 상호에서 찾습니다. 상가정보의 상호에는 지점이 붙어 오기 때문에
   (`지에스25마곡`, `이디야마곡`) 정확히 일치시키면 같은 브랜드가 수백 개로 쪼개집니다.
   표기가 갈리는 것도 한 브랜드로 모읍니다. (`파리바게뜨`/`파리바게트`, `비비큐`/`BBQ`)

   부분 일치라서 **업종을 함께 봅니다.** `씨유` 는 CU 편의점의 표기이지만
   `씨유헤어` · `아이씨유` 같은 상호에도 들어 있어, 업종을 보지 않으면
   미용실이 편의점 브랜드로 잡힙니다. 브랜드마다 들어올 수 있는 분야를 정해 두고
   맞지 않으면 붙이지 않습니다. (치킨집이 "치킨"과 "한식"에 갈려 들어오는 경우처럼
   실제로 갈리는 업종은 사전에 함께 적어 둡니다.)

2. **다점포 상호** (`brand_source = chain`) — 사전에 없지만 같은 상호가
   **행정동 5곳 이상 · 점포 10곳 이상**에 반복되는 것입니다. 지역 체인까지 사전에
   담을 수는 없어 데이터로 찾습니다. 기준을 낮추면 일반 명사가 브랜드로 둔갑합니다.
   (행정동 3곳만 요구했을 때 23,969개 → 지금 기준으로 3,059개)

   숫자 기준만으로는 `컴퓨터수리` · `입주청소` 처럼 흔한 표현이 남습니다.
   업종 분류표(대/중/소분류 명칭)에 그대로 있는 말은 상호로 보지 않습니다.

리포트의 **브랜드 목록에는 프랜차이즈만 싣습니다.** 다점포 상호는 개수로만 냅니다.
실제로 뽑아 보면 `입주청소`(643개) · `삼성공인중개사사무소`(160개) 같은 흔한 말이
상위를 차지해 브랜드 목록으로는 쓸 수 없었습니다. 체인화 정도(개수)는 뜻이 있지만
이름은 믿을 수 없습니다.

프랜차이즈 비중은 **사전에 있는 브랜드만** 센 값이라, 낮게 나오는 이유가
상권 성격 때문인지 사전이 좁아서인지 구분할 수 없습니다.
그래서 분석 문장은 개수만 적고 "개인 점포 중심 상권" 같은 판단은 쓰지 않습니다.

전국 697,147건 기준 실제 분류 결과:

| 분야 | 점포 | 프랜차이즈 | 비중 |
|---|---|---|---|
| 편의점·마트 | 31,722 | 12,766 | 40.2% |
| 패스트푸드·분식 | 29,529 | 7,567 | 25.6% |
| 카페·디저트 | 38,513 | 7,444 | 19.3% |
| 주점 | 20,682 | 774 | 3.7% |
| 식당 | 103,596 | 1,687 | 1.6% |

전문 서비스 · 의료 · 숙박 등이 0% 인 것은 그 분야 브랜드를 사전에 아직 담지 않았기 때문입니다.

결과는 `stores.sector` · `stores.brand` · `stores.brand_source` · `stores.is_franchise` 에 저장됩니다.
`sbiz:sync-stores` 로 새로 수집한 점포는 저장하는 순간 사전 매칭까지 끝나 있고,
다점포 상호(2번)는 전체를 봐야 하므로 `stores:classify` 를 한 번 더 돌려야 반영됩니다.
...
점포를 아직 수집하지 않은 지역에서는 **"0개" 대신 미수록**이라고 밝힙니다.
`sbiz:sync-stores` 를 돌리지 않은 시군구와 실제로 점포가 없는 상권은 다른 이야기이기 때문입니다.

### 행정동 이름 표기

같은 행정동이 출처마다 다르게 적혀 있습니다.
...
사람이 검색창에 치는 말은 `가양1동` 쪽이라 `Region::scopeSearch` 가 표기 차이를 흡수합니다.
숫자 앞의 `제` 를 넣고 빼 보고, 가운뎃점(`·`)과 마침표(`.`)를 바꿔 봅니다.
`제` 를 넣고 빼는 것은 **첫 숫자에만** 적용해 `제기동` 같은 진짜 이름을 망가뜨리지 않습니다.
...
**지도.** dompdf 는 JavaScript 를 실행하지 못해 Leaflet 지도가 PDF 에 들어가지 않습니다.
`App\Services\Reports\StaticMapRenderer` 가 OSM 타일을 받아 이어 붙이고 그 위에
분석 반경과 행정동 경계를 GD 로 그려 PNG 한 장으로 만듭니다.
웹 리포트도 같은 그림을 씁니다 (`/analyses/{uuid}/map.png`).

- 타일은 `storage/app/map-tiles` 에 캐시합니다. 같은 지역을 여러 번 뽑아도 다시 받지 않고,
  OSM 타일 이용 정책상으로도 그래야 합니다. 요청에는 앱을 식별하는 User-Agent 를 붙입니다.
- 타일을 못 받아도 리포트는 나옵니다. 그때는 바탕 없이 경계와 반경만 그립니다.
- 256색으로 줄여 저장합니다. 800KB 짜리 그림을 넣으면 dompdf 메모리가 위험합니다.
...
**일부만 수록된 범위.** 서울과 경기가 함께 걸리는 반경이 실제로 자주 나옵니다.
이때 유동인구는 서울 쪽 행정동에서만 잡혀 합계가 범위 전체를 대표하지 못합니다.
있음/없음만으로는 이 차이가 드러나지 않아 `meta.coverage_ratio` 에 **면적 기준 수록 비중**을
함께 담고, 100% 가 아니면 리포트에 "이 범위의 N%만 수록돼 있습니다" 라고 적습니다.
...
app/
  ...
  Services/
    ...
    Reports/              StaticMapRenderer (PDF·웹 공용 지도 그림)
